    ...
    if (phone) {
        ...
        handlers._tokens.verifyToken(token, phone, (tokenIsValid) => {
            if(tokenIsValid) {
                ...
                callback(403, {'Error' : 'No token in header or token is invalid'});
            }
        });
...
handlers._checks.post = (data, callback) => {
    ...
    if(protocol && url && method && successCodes && timeoutSeconds) {
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if(!err && userData) {
                        ...
                        if(userChecks.length < config.maxChecks){
                            ...
                            callback(400, {'Error' : `Already maxed out checks. Max checks: ${config.maxChecks}`});
                        }
                    } else {
                        callback(403);
                    }
        ...
        callback(400, {'Error' : 'Missing required inputs or inputs provided are invalid'});
    }
};
                            ...
                            if(!err) {
                                ...
                                _data.read('users', checkData.userPhone,  (err, userData) => {
                                    if(!err) {
                                        ...
                                        callback(500, {'Error' : 'User who created check not found. Check not removed from user object'});
                                    }
                                });
                            } else {
